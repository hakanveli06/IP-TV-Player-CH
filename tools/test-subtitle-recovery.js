...
(async () => {
  ...
  for (const status of [401, 410, 416, 429]) {
  ...
  {
    const env = fresh(), f = fixture();
    const net = transport(env, f, () => ({ status: 403 }));
    await assert.rejects(env.t.loadTrack(f.info, 0), e => e.status === 403);
    assert.strictEqual(net.calls.length, 2, 'Gecici medya adresi icin 403 yalnizca bir kez yenilenir');
  }
  {
    const env = fresh(), f = fixture();
