...
const controls = sandbox.Playback.vodToolControls();
assert.deepStrictEqual(Array.from(controls, x => x.id), ['tracks','subtitleSize','aspect','engine','contentInfo']);
assert.deepStrictEqual(Array.from(sandbox.Playback.vodControls(), x => x.id),
  ['play','rewind','forward','restart','favorite','details','more']);
