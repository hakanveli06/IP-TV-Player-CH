...
global.SubtitleCache = { get: function () { return null; }, getEntry: function () { return null; }, set: function () { return true; } };
const nativeSelections = [];
global.webapis = { avplay: {
  stop: function () {}, close: function () {}, setSilentSubtitle: function () {},
  getTotalTrackInfo: function () { return [{ type: 'TEXT', index: 4, extra_info: '{"track_lang":"tur"}' }]; },
  getCurrentStreamInfo: function () { return [{ type: 'TEXT', index: 4, extra_info: '{"track_lang":"tur"}' }]; },
...
global.Tx3gSubtitles = {
  inspect: function () {
    ...
    return Promise.resolve({ kind: 'mp4', sourceUrl: 'test', tracks: [{
      sourceIndex: 0, lang: 'tur', signature: 'v3:1:2:3', timescale: 1000,
      samples: [{ offset: 0, size: 8, dts: 0, cto: 0, duration: 2000 }], cues: null
    }] });
  },
  ...
  windowBackSeconds: 12, windowAheadSeconds: 120,
  _rangeCovered: function () { return false; }, cancel: function () {},
  loadWindow: function (info, index, from, to, options) {
    loads++;
    var cues = [{ start: 1, end: 2, text: 'Test' }];
    info.tracks[index].cues = cues;
    info.tracks[index].loadedRanges = [{ from: from, to: to }];
    if (options.onChunk) options.onChunk(cues, cues);
    if (options.onProgress) options.onProgress(1, 1, {});
    return Promise.resolve({ cues: cues, from: from, to: to });
  }
};
...
(async function () {
  ...
  await flush();
  assert.strictEqual(inspections, 1, 'Kaynak acik kullanici eyleminden sonra hemen incelenmeli');
  assert.strictEqual(loads, 1, 'Secilen TX3G parcasi yalnizca yakin zaman penceresi icin hazirlanmali');
  assert.strictEqual(scheduled, 0, 'Altyazi hazirlanirken oynatici kapatilip yeniden acilmamali');
  assert.strictEqual(Player.requestedSubtitlePreference(), 'lang:tr', 'Etkin secim oturum boyunca korunmali');
