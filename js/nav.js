...
function List(o) {
  ...
  this.onPosition = o.onPosition || null;
...
function Grid(o) {
  ...
  this.layoutClass = o.layoutClass || 'grid';
...
Grid.prototype.setLayout = function (cols, rowH, layoutClass) {
  var current = this.items[this.index];
  this.cols = Math.max(1, cols || this.cols);
  this.rowH = rowH || this.rowH;
  this.layoutClass = layoutClass || 'grid';
  if (current) {
...
Grid.prototype.positionInfo = function (fast) {
  var visibleRows = Math.max(1, Math.floor(this.height() / this.rowH));
  var totalRows = this.rows();
  var row = this.items.length ? Math.floor(this.index / this.cols) : 0;
  var first = this.items.length ? Math.max(0, row * this.cols) : 0;
  var last = this.items.length ? Math.min(this.items.length, first + visibleRows * this.cols) : 0;
  return {
    index: this.index, total: this.items.length, row: row, totalRows: totalRows,
    visibleRows: visibleRows, first: first, last: last, fast: !!fast
  };
};

Grid.prototype.notifyPosition = function (fast) {
  if (this.onPosition) this.onPosition(this.positionInfo(fast));
};
...
Grid.prototype.draw = function () {
  ...
  this.runner.innerHTML = html;
  this.notifyPosition(!!this._fastPosition);
  this._fastPosition = false;
};
...
Grid.prototype.page = function (delta) {
  ...
  /* Kartta tam ekran kadar satir, liste gorunumunde ise yon duygusunu
     korumak icin bir satir eksik ilerle. Her iki halde de tek oge degil,
     gorunen sayfa kapasitesi esas alinir. */
  var visibleRows = Math.max(1, Math.ceil(this.height() / this.rowH));
  var rowsPerPage = this.cols > 1 ? visibleRows : Math.max(2, visibleRows - 1);
  var column = this.index % this.cols;
  var targetRow = Math.floor(this.index / this.cols) + delta * rowsPerPage;
  targetRow = Math.max(0, Math.min(Math.max(0, this.rows() - 1), targetRow));
  this.index = Math.min(this.items.length - 1, targetRow * this.cols + column);
  this._fastPosition = true;
  this.ensure(); this.draw(); this.fire();
};
...
var Nav = {
  ...
  openIme: function (target) {
    this.imeOpen = true;
    this.imeTarget = target || document.activeElement || null;
  },

  closeIme: function (reason) {
    if (!this.imeOpen) return;
    var target = this.imeTarget;
    this.imeOpen = false;
    this.imeTarget = null;
    try { if (target && target.blur) target.blur(); } catch (e) { }
    if (this.screen && this.screen.onImeClose) this.screen.onImeClose(reason || 'close');
  },
  ...
  handle: function (e) {
    ...
    if (this.imeOpen) {
      /* Klavye acikken tuslar IME'ye aittir. Geri'yi engellemeden uygulama
         tarafindaki duzenleme kipini de ayni basista kapatiriz. */
    ...
    if (inInput) {
      ...
      if (code === KEY.ENTER) { this.openIme(ae); return; }
    }
...
document.addEventListener('change', function (e) {
  if (e.target && e.target.tagName === 'INPUT') Nav.closeIme('done');
}, true);
