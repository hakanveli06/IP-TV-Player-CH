...
var Player = {
  ...
  _softwareWindowId: 0,
  _softwareWindowLoading: false,
  _softwareReadyUntil: 0,
  _softwareLastError: '',
  ...
  _decorateSoftwareTracks: function () {
    ...
    /* Yerel MP4 servisi fazla TEXT izlerini AVPlay'e bilerek gostermez.
       Yerlesik liste bostaysa ham kaynakta bulunan TX3G izlerini uygulamanin
       cizdigi sanal secenekler olarak listeye ekle. */
    for (var s = 0; s < source.length; s++) {
      if (used[s]) continue;
      var lang = this.normalizeLanguage(source[s].lang);
      var label = lang === 'und' ? 'Altyazi ' + (s + 1) : this.languageLabel(lang);
      this.tracks.text.push({
        ...
        lang: lang,
        label: label,
        preferenceKey: lang !== 'und' ? 'lang:' + lang : 'track:' + s,
        codec: 'tx3g'
      });
      used[s] = true;
    }
  ...
  _markNativeSubtitleSuccess: function () {
    ...
    this.softwareSubtitleStatus = 'Altyazi: Dogrudan hazir';
    Diag.set('Altyazi yolu', 'Yerlesik · dogrulandi');
    if (this.cb.onNativeSubtitle) this.cb.onNativeSubtitle(this.engine);
    if (this.cb.onSubtitleStatus) this.cb.onSubtitleStatus('Altyazi: Dogrudan hazir');
  },
  ...
  _notifySubtitleUi: function () {
    if (this.cb.onSubtitleProgress) this.cb.onSubtitleProgress();
  },
  ...
  selectAudio: function (index) {
    ...
    try {
      ...
      this._playbackStaticStats = null;
  ...
  selectSubtitle: function (index) {
    ...
    if (!off && !this._softwareSubInfo && !this.live && this._engineStarted && this._readyAt) {
      if (strategyMode === 'software') {
        ...
        this._emitSubtitle('');
        this._inspectSubtitleSource(track, true);
    ...
    try {
      ...
      if (!off && !software) {
        ...
        this.softwareSubtitleStatus = strategyMode === 'unknown'
          ? 'Altyazi: Dogrudan yontem deneniyor' : 'Altyazi: Dogrudan hazir';
  ...
  canStartSubtitleCompatibility: function () {
    ...
    return !!((this.tracks && this.tracks.text && this.tracks.text.length) ||
      (this._subtitleUrl && this._subtitleUrl !== this.url));
  },
  ...
  needsSubtitleLabelDiscovery: function () {
    var list = this.tracks && this.tracks.text ? this.tracks.text : [];
    for (var i = 0; i < list.length; i++) {
      if (list[i].lang === 'und' || /^Altyazi\s+\d+$/i.test(String(list[i].label || ''))) return true;
    }
    return false;
  },
  ...
  startSubtitleCompatibility: function () {
    ...
    var track = this.tracks.subtitlesOff ? this.tracks.text[0] : this.trackByIndex('text', this.tracks.currentText);
    if (!track) {
      this.softwareSubtitleStatus = 'Altyazi: Diller inceleniyor...';
      Diag.set('Altyazi yolu', 'Yerel ham kaynakta TX3G izleri araniyor');
      return this._inspectSubtitleSource(null, false);
    }
    ...
    var enableAfter = !this.tracks.subtitlesOff;
    if (enableAfter) {
    ...
    this._inspectSubtitleSource(track, enableAfter);
    return true;
  },
  ...
  playbackStats: function () {
    ...
    try {
      if (this.engine === 'avplay' && this.hasAvplay()) {
        var fixed = this._playbackStaticStats;
        if (!fixed) {
          fixed = { width: 0, height: 0, videoCodec: '', audioCodec: '', bitrate: 0, audioBitrate: 0 };
          ...
          for (var i = 0; i < streams.length; i++) {
            ...
            if (type === 'VIDEO') {
              fixed.width = parseInt(this._infoValue(extra, ['width']), 10) || fixed.width;
              fixed.height = parseInt(this._infoValue(extra, ['height']), 10) || fixed.height;
              fixed.videoCodec = String(this._infoValue(extra, ['fourCC', 'codec']) || '');
              fixed.bitrate = parseInt(this._infoValue(extra, ['Bit_rate', 'bitrate']), 10) || fixed.bitrate;
            } else if (type === 'AUDIO') {
              fixed.audioCodec = String(this._infoValue(extra, ['fourCC', 'codec']) || '');
              fixed.audioBitrate = parseInt(this._infoValue(extra, ['bit_rate', 'bitrate']), 10) || 0;
            }
          }
          if ((!fixed.width || !fixed.height) && webapis.avplay.getVideoSize) {
            var size = webapis.avplay.getVideoSize();
            fixed.width = parseInt(size && (size.width || size.Width), 10) || fixed.width;
            fixed.height = parseInt(size && (size.height || size.Height), 10) || fixed.height;
          }
          if (fixed.width && fixed.height) this._playbackStaticStats = fixed;
        }
        out.width = fixed.width; out.height = fixed.height;
        out.videoCodec = fixed.videoCodec; out.audioCodec = fixed.audioCodec;
        out.bitrate = fixed.bitrate; out.audioBitrate = fixed.audioBitrate;
        try {
  ...
  _restoreSubtitleCache: function (sourceTrack) {
    ...
    if (!/^v[23]:/.test(String(sourceTrack.signature || ''))) return false;
    var entry = SubtitleCache.getEntry
      ? SubtitleCache.getEntry(this.cb.subtitleCacheKey, sourceTrack.sourceIndex, sourceTrack.signature)
      : null;
    var cues = entry ? entry.cues : SubtitleCache.get(this.cb.subtitleCacheKey, sourceTrack.sourceIndex, sourceTrack.signature);
    if (!cues || !cues.length) return false;
    sourceTrack.cues = cues;
    sourceTrack.loadedRanges = entry && entry.windows ? entry.windows : [];
  ...
  _storeSubtitleCache: function (sourceTrack, cues) {
    ...
    if (SubtitleCache.set(this.cb.subtitleCacheKey, sourceTrack.sourceIndex, sourceTrack.signature, cues,
        sourceTrack.loadedRanges || [])) {
      Diag.set('Altyazi onbellegi', cues.length + ' satir kaydedildi');
    }
  },

  _inspectSubtitleSource: function (track, enableAfter) {
    var self = this, token = this._sessionId;
    var preference = track && track.preferenceKey ? track.preferenceKey : this._requestedSubtitlePreference;
    if (this.live || this._softwareInspecting) return false;
    this._softwareInspecting = true;
    this._softwareLastError = '';
    this.softwareSubtitleStatus = 'Altyazi: Uyumluluk yontemiyle kaynak inceleniyor';
    this._notifySubtitleUi();
    Diag.set('Altyazi uyumlulugu', 'Oynatma kesilmeden kaynak inceleniyor');
    Tx3gSubtitles.inspect(this._subtitleUrl || this.url, function () {
      return token === self._sessionId && self.playing;
    }).then(function (info) {
      if (token !== self._sessionId) return;
      self._softwareInspecting = false;
      self._softwareSubInfo = info || { kind: 'other', tracks: [] };
      if (self._softwareSubInfo.kind !== 'mp4' || !self._softwareSubInfo.tracks.length) {
        self.softwareSubtitleStatus = 'Altyazi: Dogrudan yerlesik yontem kullaniliyor';
        Diag.set('Altyazi uyumlulugu', (self._softwareSubInfo.kind || 'other') + ' · yerlesik yol');
        self._notifySubtitleUi();
        return;
      }
      self.refreshTracks();
      var selected = self.trackByPreference('text', preference) || self.tracks.text[0];
      self.softwareSubtitleStatus = 'Altyazi: Diller hazir · Uyumluluk yontemi';
      Diag.set('Altyazi uyumlulugu', self._softwareSubInfo.tracks.length + ' TX3G dili · ilerlemeli');
      self._notifySubtitleUi();
      if (enableAfter && selected) {
        self.tracks.subtitlesOff = false;
        self.tracks.currentText = selected.index;
        self.subtitleMuted = false;
        self._requestedSubtitlePreference = selected.preferenceKey;
        self.cb.subtitlePreference = selected.preferenceKey;
        self._enableSoftwareSubtitle(selected);
      }
    })['catch'](function (error) {
      if (token !== self._sessionId || error.cancelled) return;
      self._softwareInspecting = false;
      self._softwareLastError = error.message || 'Kaynak incelenemedi';
      self.softwareSubtitleStatus = 'Altyazi hazirlanamadi · Tekrar deneyin veya baska altyazi secin';
      Diag.add('Altyazi kaynagi incelenemedi: ' + self._softwareLastError);
      self._notifySubtitleUi();
    });
    return true;
  },

  _ensureSubtitleWindow: function (position, force) {
    var self = this, sourceTrack = this._softwareSubtitleTrack;
    if (!sourceTrack || !this._softwareSubInfo || this.subtitleMuted === true || this.live) return false;
    position = Math.max(0, Number(position) || 0);
    if (!force && this._softwareReadyUntil - position > 30) return true;
    var from = Math.max(0, position - Tx3gSubtitles.windowBackSeconds);
    var to = position + Tx3gSubtitles.windowAheadSeconds;
    if (!force && sourceTrack.loadedRanges && Tx3gSubtitles._rangeCovered(sourceTrack.loadedRanges, from, to)) {
      return true;
    }
    if (this._softwareWindowLoading) return true;
    var token = this._sessionId, windowId = ++this._softwareWindowId;
    this._softwareWindowLoading = true;
    this._softwareLastError = '';
    this.softwareSubtitleStatus = 'Altyazi: Uyumluluk yontemiyle hazirlaniyor';
    this._notifySubtitleUi();
    Tx3gSubtitles.loadWindow(this._softwareSubInfo, sourceTrack.sourceIndex, from, to, {
      isActive: function () {
        return token === self._sessionId && windowId === self._softwareWindowId && self.playing;
      },
      shouldPause: function () { return self._buffering; },
      onProgress: function (done, total, status) {
        if (token !== self._sessionId || windowId !== self._softwareWindowId) return;
        Diag.set('Altyazi ilerlemeli yukleme', done + '/' + total + ' ornek · ' +
          Math.round(from) + '-' + Math.round(to) + ' sn · tek Range');
        if (status && status.message) self.softwareSubtitleStatus = 'Altyazi: ' + status.message;
        self._notifySubtitleUi();
      },
      onChunk: function (cues, batchCues, windowState) {
        if (token !== self._sessionId || windowId !== self._softwareWindowId) return;
        self._softwareSubtitleCues = cues;
        if (windowState && windowState.readyUntil) {
          self._softwareReadyUntil = Math.max(self._softwareReadyUntil || 0, windowState.readyUntil);
          self.softwareSubtitleStatus = 'Altyazi: Uyumluluk yontemiyle hazirlaniyor · ' +
            Math.max(0, Math.round(self._softwareReadyUntil - self._pos)) + ' sn hazir';
        }
        self._cueIndexSource = null;
        self._renderSoftwareSubtitle();
        self._notifySubtitleUi();
      }
    }).then(function (result) {
      if (token !== self._sessionId || windowId !== self._softwareWindowId) return;
      self._softwareWindowLoading = false;
      self._softwareSubtitleCues = result.cues;
      self._softwareReadyUntil = Math.max(self._softwareReadyUntil || 0, result.to || to);
      self.softwareSubtitleStatus = 'Altyazi: Uyumluluk yontemiyle hazir · ' +
        Math.max(0, Math.round(self._softwareReadyUntil - self._pos)) + ' sn hazir';
      self._storeSubtitleCache(sourceTrack, result.cues);
      self._markSoftwareSubtitleSuccess();
      self._renderSoftwareSubtitle();
      self._notifySubtitleUi();
    })['catch'](function (error) {
      if (token !== self._sessionId || windowId !== self._softwareWindowId || error.cancelled) return;
      self._softwareWindowLoading = false;
      self._softwareLastError = error.message || 'Altyazi okunamadi';
      self.softwareSubtitleStatus = 'Altyazi hazirlanamadi · Tekrar deneyin veya baska altyazi secin';
      Diag.add('Ilerlemeli altyazi hatasi: ' + self._softwareLastError);
      self._notifySubtitleUi();
    });
    return true;
  },

  _activateProgressiveSubtitle: function (track) {
    if (!track || !this._softwareSubInfo || track.softwareIndex == null) return false;
    var sourceTrack = this._softwareSubInfo.tracks[track.softwareIndex];
    if (!sourceTrack || sourceTrack.manifestOnly || !sourceTrack.samples || !sourceTrack.samples.length) {
      return this._inspectSubtitleSource(track, true);
    }
    if (this._softwareSubtitleTrack && this._softwareSubtitleTrack !== sourceTrack && this._softwareWindowLoading) {
      this._softwareWindowId++;
      this._softwareWindowLoading = false;
    ...
    this._restoreSubtitleCache(sourceTrack);
    this._softwareSubtitleTrack = sourceTrack;
    this._softwareSubtitleCues = sourceTrack.cues || [];
    this._softwareReadyUntil = 0;
    var ranges = sourceTrack.loadedRanges || [];
    for (var r = 0; r < ranges.length; r++) {
      if (ranges[r].from <= this._pos && ranges[r].to > this._softwareReadyUntil) this._softwareReadyUntil = ranges[r].to;
    }
    this._softwareDesiredIndex = track.index;
    this._softwareLastText = '';
    this._cueIndexSource = null;
    if (this._softwareReadyUntil - this._pos > 30) {
      this.softwareSubtitleStatus = 'Altyazi: Uyumluluk yontemiyle hazir · ' +
        Math.round(this._softwareReadyUntil - this._pos) + ' sn hazir';
      this._renderSoftwareSubtitle();
      this._markSoftwareSubtitleSuccess();
      this._notifySubtitleUi();
    }
    this._ensureSubtitleWindow(this._pos, false);
    return true;
  },
  ...
  _prepareSelectedTrackSafely: function (track) {
    return this._activateProgressiveSubtitle(track);
  },
  ...
  retrySubtitlePreparation: function () {
    ...
    var subtitleUrl = this._subtitleUrl || this.url;
    var until = Tx3gSubtitles._cooldowns && Tx3gSubtitles._cooldowns[subtitleUrl];
    if (until > Date.now()) {
  ...
  _cancelSoftwareSubtitle: function () {
    if ((this._softwareWindowLoading || this._softwareInspecting) &&
        typeof Tx3gSubtitles !== 'undefined' && Tx3gSubtitles.cancel) Tx3gSubtitles.cancel();
    this._softwareLoadId++;
    this._softwareWindowId++;
  ...
  play: function (url, opts) {
    ...
    this.paused = false;
    this.url = url;
    this._subtitleUrl = opts.subtitleSourceUrl || url;
    this.live = !!opts.live;
    this.cb = opts;
    ...
    this._bufferProgressAt = 0;
    this._lastBufferAspectAt = 0;
  ...
  errorText: function (error, prefix) {
    var parts = [], text = '';
    if (error && typeof error === 'object') {
      if (error.name) parts.push(String(error.name));
      if (error.code != null && String(error.code) !== '') parts.push('code=' + String(error.code));
      if (error.message) parts.push(String(error.message));
    } else if (error != null) text = String(error);
    if (parts.length) text = parts.join(' · ');
    if (!text) text = 'Bilinmeyen hata';
    return prefix ? prefix + ': ' + text : text;
  },

  isCompatibilityError: function (error) {
    return /not.support|unsupported|invalid.stream|codec|format|bicim|cozumlenemedi/i.test(this.errorText(error));
  },

  _fail: function (msg, canFallback) {
    msg = this.errorText(msg);
    if (this._failed) return;
    if (this.live && canFallback && !this.isCompatibilityError(msg)) {
      canFallback = false;
    }
    ...
    try {
      ...
      webapis.avplay.setListener({
        ...
        onbufferingcomplete: function () {
          ...
          var now = Date.now();
          if (!self._lastBufferAspectAt || now - self._lastBufferAspectAt >= 30000) {
            self._lastBufferAspectAt = now;
            self._playbackStaticStats = null;
            self.applyAspect();
          }
        ...
        onbufferingprogress: function (percent) {
          ...
          if (percent > self._bufferProgress) {
            self._bufferProgress = percent;
            self._bufferProgressAt = Date.now();
          }
        },
        ...
        onerror: function (err) {
          if (token !== self._sessionId) return;
          UI.spin(false);
          self._fail(self.errorText(err, 'AVPlay hatasi'), true);
        },
      ...
      webapis.avplay.prepareAsync(function () {
        ...
        if (token !== self._sessionId) return;
        UI.spin(false);
        var detail = self.errorText(err, 'Yayin hazirlanamadi');
        Diag.add('AVPlay prepare hatasi: ' + detail);
        self._fail(detail, true);
      });
    } catch (e) {
      UI.spin(false);
  ...
  _bufferStart: function (token, source) {
    ...
    if (!this._buffering) {
      ...
      this._bufferProgress = 0;
      this._bufferProgressAt = this._bufferingSince;
    ...
    if (this.live && this._readyAt && !this.paused) {
      var self = this;
      var resume = this.cb && this.cb.buffer ? Number(this.cb.buffer.resume) || 5 : 5;
      var waitMs = this.engine === 'html5' ? 14000 : Math.max(10000, (resume + 6) * 1000);
      function watchBuffer() {
        if (token !== self._sessionId || !self._buffering) return;
        /* AVPlay yuzdesi ilerliyorsa baglanti canlidir; biriktirdigi tamponu
           kesip yeniden acmak yerine ona zaman ver. */
        if (Date.now() - self._bufferProgressAt < 6500) {
          self._bufferTimer = setTimeout(watchBuffer, 6500);
          return;
        }
        self._emitStall('tampon ilerlemesi durdu');
      }
      this._bufferTimer = setTimeout(watchBuffer, waitMs);
    }
  },
  ...
  _bufferEnd: function (token, source) {
    ...
    if (this._softwareSubtitleTrack) this._ensureSubtitleWindow(this._pos, false);
  },
  ...
  seek: function (deltaSec) {
    ...
    if (this._softwareSubtitleCues) {
      ...
      this._softwareWindowLoading = false;
      if (typeof Tx3gSubtitles !== 'undefined' && Tx3gSubtitles.cancel) Tx3gSubtitles.cancel();
      this._ensureSubtitleWindow(target, true);
    }
  ...
  _startTick: function (token) {
    ...
    this._tick = setInterval(function () {
      ...
      var liveClockLimit = self.engine === 'html5' ? 14000 : Math.max(10000,
        (((self.cb && self.cb.buffer) ? Number(self.cb.buffer.resume) : 5) || 5) * 1000 + 6000);
      if (self.live && self._readyAt && !self._buffering && !self.paused &&
        now - self._lastProgressAt > liveClockLimit) {
