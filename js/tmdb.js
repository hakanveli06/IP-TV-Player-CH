...
var Tmdb = {
  ...
  proxyBase: function () {
    var value = '';
    try { value = String((window.HM_RUNTIME && window.HM_RUNTIME.tmdbProxyBase) || ''); }
    catch (e) { value = ''; }
    return value.replace(/^\s+|\s+$/g, '').replace(/\/+$/, '');
  },
  proxyConfigured: function () { return /^https?:\/\//i.test(this.proxyBase()); },
  userCredential: function () {
    try { return this.normalizeCredential(typeof Settings !== 'undefined' ? Settings.get('tmdbCredential') : ''); }
    catch (e) { return ''; }
  },
  credential: function () { return this.userCredential(); },
  configured: function () { return !!this.credentialKind(this.userCredential()) || this.proxyConfigured(); },
  source: function () {
    if (this.userCredential()) return 'user';
    if (this.proxyConfigured()) return 'proxy';
    return 'none';
  },
  sourceLabel: function () {
    var source = this.source();
    return source === 'user' ? this.text('tmdb.personalActive', 'Kişisel anahtar etkin') :
      (source === 'proxy' ? this.text('tmdb.proxyActive', 'Public TMDb hizmeti etkin') :
  ...
  requestWithProxy: function (path, params) {
    var self = this;
    return new Promise(function (resolve, reject) {
      var base = self.proxyBase();
      if (!/^https?:\/\//i.test(base)) {
        reject(new Error('Public TMDb hizmeti henüz yapılandırılmamış.'));
        return;
      }
      var query = [], key;
      params = params || {};
      ...
      var xhr = new XMLHttpRequest();
      xhr.open('GET', base + path + (query.length ? '?' + query.join('&') : ''), true);
      xhr.timeout = 18000;
      xhr.setRequestHeader('Accept', 'application/json');
      xhr.onload = function () {
        var data = null;
        try { data = JSON.parse(xhr.responseText || '{}'); } catch (e) { }
        if (xhr.status >= 200 && xhr.status < 300 && data) { resolve(data); return; }
        if (xhr.status === 404) reject(new Error('TMDb kaydı bulunamadı.'));
        else if (xhr.status === 429) reject(new Error('TMDb hizmeti geçici olarak yoğun.'));
        else reject(new Error('TMDb bilgisi şu anda alınamadı.'));
      };
      xhr.onerror = function () { reject(new Error('Public TMDb hizmetine bağlanılamadı.')); };
      xhr.ontimeout = function () { reject(new Error('TMDb isteği zaman aşımına uğradı.')); };
      xhr.send();
    });
  },
  request: function (path, params) {
    var credential = this.userCredential();
    return this.credentialKind(credential) ? this.requestWithCredential(path, params, credential) :
      this.requestWithProxy(path, params);
  },
  ...
  loadCache: function () {
    ...
    if (!data || data.version !== 5 || !data.items || !(data.order instanceof Array)) {
      data = { version: 5, items: {}, order: [] };
    }
    this._cache = data;
    return data;
  },
      ...
      for (var i = 0; i < definitions.length; i++) {
        ...
        for (var j = 0; j < rows.length; j++) {
          ...
          providers.push({
            ...
            logo: rows[j].logo_path ? 'https://image.tmdb.org/t/p/w92' + rows[j].logo_path : ''
          });
        }
  ...
  season: function (tvId, seasonNumber) {
    var self = this, locale = this.locale();
    var key = 'season:' + this.localeScope() + ':' + tvId + ':' + seasonNumber;
    var cached = this.cacheGet(key);
    if (cached) return Promise.resolve(cached);
    return this.request('/tv/' + tvId + '/season/' + seasonNumber, { language: locale }).then(function (data) {
      var rows = data.episodes || [], result = {};
      for (var i = 0; i < rows.length; i++) {
        var ep = rows[i], no = String(ep.episode_number || (i + 1));
        result[no] = {
          title: ep.name || '', overview: ep.overview || '', date: ep.air_date || '',
          runtime: +ep.runtime || 0, rating: +ep.vote_average || 0,
